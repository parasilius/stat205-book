...
\begin{exmp}\label{exmp:playing_cards}
	...
	Each time a card is drawn, it is returned to the deck and the deck is shuffled, so the conditions of each experiment remains the same.
...
By definition, \( P(E | F) = \frac{P(E \cap F)}{P(F)} \) which implies \( P(E \cap F) = P(E | F)P(F) \), also known as \keyterm{multiplication rule} (not to be confused with the multiplication principle!).
