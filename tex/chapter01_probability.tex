...
The \keyterm{complement} of an event \( E \) is the difference \( S \setminus E \), containing all elements in sample space that are not in \( E \).
...
An example is shown in \autoref{fig:frequentistprobability}\FOOTNOTE{\url{https://github.com/tomfaulkenberry/JASPbook}}.

\begin{figure}[p]
\begin{center}
\epsfig{file=../img/frequentistProb.png,clip=true,width=14cm}
...
\begin{exmp}\label{exmp:fair_die_even}
...
In the previous examples, we can easily count the elements in both the sample space and the event of interest.
...
Any arrangement of a set of \( n \) distinct objects (or actions or anything!) is called a \keyterm{permutation}.
To enumerate the number of permutations, we utilize the multiplication principle.
Any arrangement can be created sequentially.
First, we select one of the \( n \) objects, then one of the \( n - 1 \) remaining ones,
and we continue this.
So the number of permutations is \( n \times (n - 1) \times \ldots \times 2 \times 1 = n! \).
\begin{exmp}
	There are 8! ways to paint 8 distinct rooms.
\end{exmp}
Now consider selecting and ordered sequence of \( r \) objects from \( n \) distinct objects.
We first choose the first object which can be done in \( n \) ways, then the second one which can be done in \( n - 1 \) ways,
and so on until selecting the \( r \)-th object which can be done in \( n - (r - 1) \) ways.
Therefore, the total number of possible sequences is:
\begin{gather*}
	P_{n, r} = n \times (n - 1) \times \ldots (n - (r -  1)) = \frac{n!}{(n - r)!}
\end{gather*}

\begin{exmp}
	There are \( P_{9, 3} = \frac{9!}{(9 - 3)!} = 504 \)  ways that a manager can select and assign 3 employees from 9 to operate 3 different machines (one employee per machine).
\end{exmp}

\subsection{Combination}

In the previous subsection, we considered ordered arrangements of objects.
Now suppose we want to select \( r \) objects from \( n \) distinct ones without regard to ordering.
How many distinct selections are possible?

Suppose the answer is \( C_{n, r} \).
So we can choose \( r \) objects from \( n \) distinct objects in \( C_{n, r} \) ways.
But let's assume that after choosing these \( r \) objects, we are once again interested in their ordering.
Regardless of the previous step, there are \( r! \) ways to arrange these \( r \) objects.
However, in the previous subsection, we calculated the total number of ways to choose an ordered sequence of \( r \) objects from \( n \) distinct objects in a single step,
which is \( P_{n, r} \).
This yields:
\begin{gather*}
	C_{n, r} \times r! = P_{n, r}\\
	C_{n, r} = \frac{P_{n, r}}{r!}\\
	C_{n, r} = \frac{n!}{r!(n - r)!}
\end{gather*}
An alternative notation for the combination \( C_{n, r} \) is the binomial coefficient \( n \choose r \).
\begin{exmp}
	A selection committee must choose 6 players from a team of 7 basketball players and a team of 8 soccer players.
	What is the probability that the same number of players is chosen from each team?
\end{exmp}
\begin{solution}
	The total number of ways to choose 6 players from 15 players is \( 15 \choose 6 \).
	On the other hand, there are \( 7 \choose 3 \) ways to choose 3 basketball players,
	and for each group of 3 basketball players, there are \( 8 \choose 3 \) ways to choose 3 soccer players.
	So the desired probability is \( \frac{\binom{7}{3} \times \binom{8}{3}}{\binom{15}{6}} \approx 0.39 \).
\end{solution}
...
The \keyterm{conditional probability} of \( A \) given \( B \), denoted \( P(A | B) \), is the probability of \( A \) occurring when \( B \) is already known to have occurred.
...
\begin{figure}[t]
\begin{center}
\epsfig{file=../img/conditional_prob.jpeg,clip=true,width=8.8cm}
\end{center}
...
By definition, \( P(E | F) = \frac{P(E \cap F)}{P(F)} \) which implies \( P(E \cap F) = P(F)P(E | F) \), also known as \keyterm{multiplication rule} (not to be confused with the multiplication principle!).
...
Note that two disjoint events are not necessarily independent.
...
\end{gather*}
which is known as \keyterm{Bayes' rule} or \keyterm{Bayes' thorem}.
...
Sometimes we are interested in the proportion of a population that has a characteristic.
