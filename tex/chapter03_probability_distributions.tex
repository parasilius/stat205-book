...
[1] 0.1573054
    ...
    \end{enumerate}
\end{solution}
Now suppose that we want to find \( \bm{x_0} \) such that \( P(\bm{X} \leq \bm{x_0}) = p \) where \( p \in [0, 1] \).
...
However, any normally distributed random variable \( \bm{X} \sim N(\mu, \sigma) \) can be converted to a \keyterm{standard normal random variable} \( \bm{Z} \sim N(0, 1) \).
...
\( z_1 \) and \( z_2 \) are called the \keyterm{standardized scores} or \keyterm{z-scores} of \( \bm{x} \) and \( \bm{y} \), respectively.
...
Converting the normal variable to standard variable is called \keyterm{standardization}, and can be used for such comparisons.
