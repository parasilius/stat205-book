...
We say \( \bm{X} \) follows a binomial distribution and denote this by \( \bm{X} \sim B(n, p) \),
where \( p \) is the probability of success and is the same in each Bernoulli trial.
...
\begin{solution}
    ...
    Therefor, \( \bm{Y} \sim B(40, 0.89) \) with PMF:
    \begin{gather*}
...
[1] 0.1794093
    \end{lstlisting}
\end{solution}
We may be interested in \( P(\bm{X} \leq q) \), where \( q \) can be any real number.
It is obvious that if \( q < 0 \), \( P(\bm{X} \leq q) = 0 \), since \( \bm{X} \) is defined for integers starting at zero.
For \( q \geq 0\), \( P(\bm{X} \leq q) = p_{\bm{X}}(0) + \ldots + p_{\bm{X}}(\floor{q}) \).
